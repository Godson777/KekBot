...
public class Gru extends Command {
        ...
        if (event.getMessage().getAttachments().size() > 0) {
            if (event.getMessage().getAttachments().get(0).isImage()) {
                try {
                    ...
                    event.getChannel().sendFile(generate(ImageIO.read(event.getMessage().getAttachments().get(0).getInputStream()), hyper),  filename + ".png", null).queue();
                } catch (IOException e) {
            ...
            if (event.getArgs().length > 0) {
                ...
                try {
                    ...
                    if (check == null) {
                        try {
                            event.getChannel().sendFile(generate(event.combineArgs().replaceAll("--hyper", ""), hyper), filename + ".png", null).queue();
                    ...
                    event.getChannel().sendFile(generate(check, hyper), filename + ".png", null).queue();
    ...
    private byte[] generate(BufferedImage image, boolean hyper) throws IOException {
        BufferedImage base = ImageIO.read(new File("resources/memegen/grusmasterplan" + (hyper ? "hyper" : "") + ".png"));
        ...
        graphics.drawImage(image, 436 + a, 114 + b, dimension.width, dimension.height, null);
        graphics.drawImage(image, 1191 + a, 114 + b, dimension.width, dimension.height, null);
        graphics.drawImage(image, 442 + a, 595 + b, dimension.width, dimension.height, null);
        graphics.drawImage(image, 1190 + a, 595 + b, dimension.width, dimension.height, null);
        if (hyper) graphics.drawImage(image, 786 + a, 1073 + b, dimension.width, dimension.height, null);
        graphics.dispose();
    ...
    private byte[] generate(String string, boolean hyper) throws IOException {
        BufferedImage base = ImageIO.read(new File("resources/memegen/grusmasterplan" + (hyper ? "hyper" : "") + ".png"));
        Graphics2D graphics = base.createGraphics();
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            try {
                ...
                if (i == 0) graphics.drawImage(check, 436 + a, 114 + b, dimension.width, dimension.height, null);
                if (i == 1) graphics.drawImage(check, 1191 + a, 114 + b, dimension.width, dimension.height, null);
                if (i == 2) {
                    graphics.drawImage(check, 442 + a, 595 + b, dimension.width, dimension.height, null);
                    graphics.drawImage(check, 1190 + a, 595 + b, dimension.width, dimension.height, null);
                    if (hyper) graphics.drawImage(check, 786 + a, 1073 + b, dimension.width, dimension.height, null);
                }
                ...
                if (i == 0) {
                    drawString(graphics, text.toString(), 436 + a, 114 + b - 50);
                }
                if (i == 1) {
                    drawString(graphics, text.toString(), 1191 + a, 114 + b - 50);
                }
                if (i == 2) {
                    drawString(graphics, text.toString(), 442 + a, 595 + b - 50);
                    drawString(graphics, text.toString(), 1190 + a, 595 + b - 50);
                    if (hyper) drawString(graphics, text.toString(), 786 + a, 1073 + b - 50);
                }
